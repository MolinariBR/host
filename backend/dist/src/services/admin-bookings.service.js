...
export async function updateAdminBooking(prisma, bookingId, input) {
    const current = await bookingsRepo.getAdminBookingById(prisma, bookingId);
    if (!current)
        throw new BookingNotFoundError("Resource not found.");
    if (input.status &&
        !isValidBookingStatusTransition(current.status, input.status)) {
        throw new InvalidBookingStatusTransitionError(`Invalid status transition from ${current.status} to ${input.status}.`);
    }
