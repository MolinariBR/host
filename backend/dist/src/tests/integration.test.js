...
async function getFirstAvailableRoom() {
    ...
    const body = roomsResponse.json();
    assert.ok(body.items.length > 0, "expected at least one available room");
    return body.items[0];
}
async function createBookingForEmail(email) {
...
test("criacao de reserva publica retorna bookingCode e link de WhatsApp", async () => {
    ...
    assert.match(booking.bookingCode, new RegExp(`^${bookingCodePrefix}-\\d{8}-\\d{3,}$`));
...
test("lookup de reservas por email retorna itens", async () => {
    ...
    const body = lookupResponse.json();
...
test("bloqueia transicao invalida de status da reserva", async () => {
    ...
    const listBody = adminListResponse.json();
