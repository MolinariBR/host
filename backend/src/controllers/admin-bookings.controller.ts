...
export async function listBookingsController(request: FastifyRequest, reply: FastifyReply) {
...
export async function getBookingByIdController(request: FastifyRequest, reply: FastifyReply) {
  ...
  if (!booking) return reply.code(404).send({ message: "Resource not found." });

  return reply.code(200).send(booking);
}

export async function updateBookingController(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    const booking = await adminBookingsService.updateAdminBooking(
      prisma,
