...
export async function getSummaryReport(
  ...
  const occupancyRate =
    totalCapacityNights > 0 ? Number(((occupiedNights / totalCapacityNights) * 100).toFixed(2)) : 0;

  const byRoomMap = new Map<
    string,
    { roomId: string; roomNumber: string; bookings: number; revenueCents: number; occupiedNights: number }
  >();
