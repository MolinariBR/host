import type { PrismaClient, BookingStatus, PaymentStatus } from "@prisma/client";
...
function resolveHotelWhatsappPhone(profilePhone?: string | null): string {
...
export async function lookupPublicBookings(
  prisma: PrismaClient,
  input: { email: string; bookingCode?: string }
) {
...
export async function getPublicBookingWhatsappLink(
  prisma: PrismaClient,
  bookingCode: string
): Promise<{ url: string }> {
...
export async function createPublicBookingWithGlobalClient(input: CreatePublicBookingInput) {
